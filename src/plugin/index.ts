...
function validateEnvironment(ports: PortsFile) {
  if (!process.env.HTTP_PROXY) {
    ...
    throw new Error(
      "HTTP_PROXY environment variable not set. Make sure cypress is started using the cypress-ntlm launcher."
    );
  }
  if (process.env.HTTP_PROXY !== ports.ntlmProxyUrl) {
    debug.log(
      "Error: HTTP_PROXY environment variable (" +
        process.env.HTTP_PROXY +
        ") " +
        "is not set to current NTLM proxy url (" +
        ports.ntlmProxyUrl +
        ")."
    );
    throw new Error(
      "HTTP_PROXY environment variable is not set to " +
        "current NTLM proxy url (" +
        ports.ntlmProxyUrl +
        "). " +
...
function setupProxyEnvironment(config: ICypressPluginConfig, ports: PortsFile) {
  config.env.NTLM_AUTH_PROXY = ports.ntlmProxyUrl;
  config.env.NTLM_AUTH_API = ports.configApiUrl;
  return config;
}

interface ICypressPluginConfig {
  env: {
