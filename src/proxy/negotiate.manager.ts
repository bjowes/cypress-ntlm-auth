...
export class NegotiateManager implements INegotiateManager {
  ...
  private handshakeResponse(res: http.IncomingMessage, ntlmHostUrl: CompleteUrl, context: IConnectionContext,
    ...
    if (!responseToken && res.statusCode === 401) {
      this._debug.log('Negotiate authentication failed for host, invalid credentials', ntlmHostUrl.href);
      context.setState(ntlmHostUrl, NtlmStateEnum.NotAuthenticated);
      return callback(undefined, res);
    }
    ...
    if (requestOptions.headers) { // Always true, silent the compiler
