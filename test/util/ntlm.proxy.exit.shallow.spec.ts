...
describe("NtlmProxyExit shallow", () => {
  ...
  beforeEach(function () {
    ...
    ntlmProxyExit = new NtlmProxyExit(debugMock);
  });
  ...
  it("should send quit to existing proxy", async function () {
    ...
    await ntlmProxyExit.quitIfRunning(fakeConfigApiUrl);

    expect(httpRequestStub.calledOnce).to.be.true;
    expect(callUrl).to.be.equal(fakeConfigApiUrl + "/quit");
    expect(callBody).to.be.deep.equal({ keepPortsFile: false });
    expect(callOptions).to.be.deep.equal({ timeout: 5000 });
    debugMock
      .received(1)
      .log("ntlm-proxy-exit: Sending shutdown command to NTLM proxy");
  });
  ...
  it("should not send quit if no existing proxy", async function () {
    ...
    debugMock
      .received(1)
      .log("ntlm-proxy-exit: CYPRESS_NTLM_AUTH_API is not set, nothing to do.");
  });
  ...
  it("should throw if quit throws", async function () {
    ...
    await expect(
      ntlmProxyExit.quitIfRunning(fakeConfigApiUrl)
    ).to.be.rejectedWith("Shutdown request failed: " + "Error: test error");
    debugMock
      .received(1)
      .log("ntlm-proxy-exit: Shutdown request failed: " + "Error: test error");
  });
  ...
  it("should throw if quit returns != 200", async function () {
    ...
    debugMock
      .received(1)
      .log("ntlm-proxy-exit: Unexpected response from NTLM proxy: 404");
  });
});
