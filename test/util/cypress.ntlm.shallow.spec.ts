...
describe('CypressNtlm shallow', () => {
  ...
  let httpRequestStub: sinon.SinonStub<[string, AxiosRequestConfig?], Promise<{}>>;
  ...
  it('should send alive to existing proxy', async function () {
    ...
    debugMock.received(0).log('Older ports file present. Waiting 2000 ms for new proxy instance to remove it.');
  ...
  it('should check for ports file multiple times', async function () {
    ...
    portsFileServiceMock.recentlyModified().mimicks(() => { recentlyModifiedCalls++; return false; });
    let existsCalls = 0;
    portsFileServiceMock.exists().mimicks(() => { existsCalls++; return false; });

    await expect(cypressNtlm.checkProxyIsRunning(3000, 400)).to.be.rejectedWith('Ports file not found before timed out');

    expect(recentlyModifiedCalls).to.be.equal(1);
    expect(existsCalls).to.be.at.least(2);
    debugMock.received(1).log('Older ports file present. Waiting 2000 ms for new proxy instance to remove it.');
  });
  ...
  it('should retry if parsing of ports file fails', async function () {
    let recentlyModifiedCalls = 0;
    portsFileServiceMock.recentlyModified().mimicks(() => { recentlyModifiedCalls++; return true; });
    let existsCalls = 0;
    portsFileServiceMock.exists().mimicks(() => { existsCalls++; return true; });
    let parseCalls = 0;
    portsFileServiceMock.parse().mimicks(() => { parseCalls++; throw new Error('test'); });

    await expect(cypressNtlm.checkProxyIsRunning(1000, 400)).to.be.rejectedWith('Ports file not found before timed out');

    expect(recentlyModifiedCalls).to.be.equal(1);
    expect(existsCalls).to.be.at.least(2);
    expect(parseCalls).to.be.at.least(2);

    debugMock.received().log('Failed to parse ports file. May just have been removed. Retrying...');
  });
  ...
  it('should retry if contacting proxy fails', async function () {
    ...
    portsFileServiceMock.exists().mimicks(() => { existsCalls++; return true; });
    let parseCalls = 0;
    portsFileServiceMock.parse().mimicks(() => { parseCalls++; return portsFile; });
    ...
    debugMock.received().log('Invalid response from ntlm-proxy. May just have been removed. Retrying...');
  });
  ...
  it('should retry if contacting proxy rejects', async function () {
    ...
    debugMock.received().log('Failed to contact ntlm-proxy. May just have been removed. Retrying...');
  });
});
