...
describe('urlConverter', function() {
  describe('toCompleteUrl', function () {
    ...
    it('should throw for undefined url', function() {
      // Act
      chai.expect(() => toCompleteUrl(undefined, false)).to.throw('Could not parse empty host');
    });

    it('should throw for empty url', function() {
      // Act
      chai.expect(() => toCompleteUrl('', false)).to.throw('Could not parse empty host');
    });

    it('should throw for blank url', function() {
      // Act
      chai.expect(() => toCompleteUrl(' ', false)).to.throw('Missing mandatory properties of complete url: ');
    });
    ...
    it('should set href to hostname and path for url with path', function() {
      ...
      // Assert
      chai.expect(result.href).to.equal('http://localhost:80/nisse/bosse?olle=pelle&karl=oskar');
    });
    ...
    it('should throw if useSSL argument is not set when addProtocol is set', function() {
      // Act
      chai.expect(() => toCompleteUrl('127.0.0.1:80', true)).to.throw('Must specify useSSL parameter when addProtocol is set')
    });
